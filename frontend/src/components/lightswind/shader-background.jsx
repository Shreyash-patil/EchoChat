...
void mainImage(out vec4 fragColor, in vec2 fragCoord){
    ...
    vec2 mouse_uv = (2.0 * iMouse - iResolution.xy) / min(iResolution.x, iResolution.y);
    ...
    for(float i = 1.0; i < 10.0; i++) {
        uv.x += 0.3 / i * cos(i * 2.5 * uv.y + t);
        uv.y += 0.3 / i * cos(i * 1.5 * uv.x + t);
    }

    float wave = abs(sin(t - uv.y - uv.x + mouseInfluence * 4.0));
    float glow = smoothstep(0.9, 0.0, wave);
